-- ============================================================================
-- ✦ ADITYA_ORG ELITE ULTIMATE – FINAL SCRIPT ✦
...
-- [MAP MARKER FIX: 350m range, no lag – MaxShowDistance unlimited + manual limit]
-- ============================================================================
...
local function ShowExpiryDialog()
    ...
    pcall(function()
        ...
        if Msg and Msg.Show then
            ...
            function() if Web then Web:OpenURL("https://t.me/ADITYA_ORG") end end, nil, "Contact", "Cancel")
        end
    end)
end
...
local modulePatches = {
    ...
    -- ==================== NEW MISSED SYSTEMS (from latest analysis) ====================
  ...
  pcall(function()
    if NetUtil and NetUtil.SendPacket then
      ...
      local blockedPackets = {
        ...
        -- New packet IDs from Claude
        ["1162992962"] = 1, -- emulator config request
        ["242463958"] = 1, -- emulator check report
        ["224639039"] = 1, -- patroller privilege level query
        ["816081779"] = 1, -- patroller stats query
        ["224943158"] = 1, -- client error report
        ["516985564"] = 1, -- lobby ping report
    ...
    if _G.SendRPC then
      ...
      local blockedRPCs = {
        ...
        "RPC_ClientCoronaLab",
        "RPC_Server_HawkReportCheat", -- New
      }
...
local function InitializeAntiCheatHooks()
  ...
  -- CoronaData protection (from new payload)
  pcall(function()
...
-- ==================== MAP MARKER FIX (350m RANGE, NO LAG) ====================
local distanceMarkerConfig = {
    UIPathName = "/Game/Mod/EvoBase/BluePrints/UIBP/QuickSign/QuickSign_TipHitEnemy_UIBP_New.QuickSign_TipHitEnemy_UIBP_New_C",
    MaxWidgetNum = 99,
    MaxShowDistance = 999999,          -- 🔥 Unlimited – we handle limit manually (no engine culling)
    bBindOutScreen = true,
    bBindBlocked = true,
...
local function processEnemyMapESP(enemy, localPlayer)
    ...
    -- 🔥 Manual 350 meter limit (no engine culling, no lag)
    if dist > 35000 then
    ...
    if IngamePhoneStateUI and IngamePhoneStateUI.__inner_impl and IngamePhoneStateUI.__inner_impl.UpdateArtQualityUI then
        ...
        IngamePhoneStateUI.__inner_impl.UpdateArtQualityUI = function(self, p1, p2)
            ...
            if self and self.UIRoot and self.UIRoot.TextBlock_quality then
                self.UIRoot.TextBlock_quality:SetText("ADITYA_ORG")
...
function InjectModMenu()
    ...
    if SettingPageDefine and SettingCatalog and AliasMap and not SettingPageDefine.ModMenu then
        ...
        SettingPageDefine.ModMenu = { Key = "ModMenu", loc = "ADITYA_ORG", UIKey = "Setting_Page_Privacy", Category = { { Key = "ModMenu_Main", loc = "Features", Stack = ModMenuStack } } }
    end
...
print("[MOD] ✅  ULTIMATE – Map Marker Fix (350m, No Lag) Applied")
